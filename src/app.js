...
const SUMMARY_RULES_PROMPT = `[redacted in public source package]`;

const SUMMARY_REVIEW_RULES_PROMPT = `[redacted in public source package]`;
...
async function transcribeRawTextWithGemini(mediaParts, options = {}) {
  const raw = await callGemini([
    ...mediaParts,
    {
      text: `[redacted transcription prompt in public source package]`,
    },
  ], "text/plain", options);
...
async function rephraseWithGemini(transcriptText = formatTranscript(), options = {}) {
  ...
  const raw = await callGemini([
    {
      text: `[redacted summary prompt in public source package]`,
    },
...
async function analyzeSummaryIssuesWithGemini(summaryText, options = {}) {
  ...
  const raw = await callGemini([
    {
      text: `[redacted review prompt in public source package]`,
    },
  ], "application/json", options);
        ...
        <div class="audio-card player-card">
          <div class="audio-card-text">
            <h2 class="latest-audio">
              ...
              <strong>${state.recordedAudioUrl ? "พร้อมสำหรับการถอดเสียง" : "ยังไม่มีเสียง"}</strong>
...
function bindEvents() {
  ...
  document.querySelector("#summary-text")?.addEventListener("input", (event) => {
    ...
    const changed = nextSummary !== state.summary;
  ...
  document.querySelectorAll(".summary-issue").forEach((button) => {
    button.addEventListener("click", (event) => {
      ...
      const suggestions = getWordSuggestions(word);
      const displayWord = String(button.textContent || issue.text || word.word || "").trim();
      ...
      state.issueNotice = {
        ...
        suggestions,
        word: displayWord,
        dismissTexts: [displayWord, word.word, word.originalWord, ...suggestions],
      };
      render();
    });
  });
